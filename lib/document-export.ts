...
async function exportPdf(document: GeneratedDocument) {
  ...
  const hasCjk = containsCjkText(pdfFontProbeText);
  const embeddedFont = await loadPdfFontBytes(pdfFontProbeText);

  let font: PDFFont;
  if (!embeddedFont) {
    console.warn("[PDF Export] ⚠️ No font file found, using Helvetica. Chinese characters WILL NOT display.");
    font = await pdf.embedFont(StandardFonts.Helvetica);
  } else {
    // For CJK fonts, subset embedding often produces invisible glyphs.
    // Try subset:false first (larger file but guaranteed visible),
    // fall back to subset:true, then Helvetica.
    let embeddedFont2: PDFFont | null = null;

    if (hasCjk) {
      try {
        embeddedFont2 = await pdf.embedFont(embeddedFont.bytes, { subset: false });
        console.log(`[PDF Export] ✅ Font embedded (full, no subset): ${embeddedFont.path}`);
      } catch (error) {
        console.warn(`[PDF Export] ⚠️ Full font embed failed, trying subset. Error: ${String(error)}`);
      }
    }

    if (!embeddedFont2) {
      try {
        embeddedFont2 = await pdf.embedFont(embeddedFont.bytes, { subset: true });
        console.log(`[PDF Export] ✅ Font embedded (subset): ${embeddedFont.path}`);
      } catch (error) {
        console.warn(`[PDF Export] ❌ Subset font embed failed: ${embeddedFont.path}. Error: ${String(error)}`);
      }
    }

    if (!embeddedFont2) {
      console.warn("[PDF Export] ❌ All font embed attempts failed. Falling back to Helvetica.");
      embeddedFont2 = await pdf.embedFont(StandardFonts.Helvetica);
    }

    font = embeddedFont2;
  }
  console.log(`[PDF Export] Document title: "${document.title}", sections: ${document.sections.length}, hasCjk: ${hasCjk}`);
  const marginX = 50;
  const maxWidth = 495;
  ...
  console.log(`[PDF Export] Generated ${pdfBytes.length} bytes, pages: ${pdf.getPageCount()}`);
  return pdfBytes;
}

...
async function loadOverlayFont(pdf: PDFDocument, text: string) {
  ...
  const hasCjk = /[^\u0000-\u00ff]/.test(text);

  if (loadedFont) {
    // For CJK text, try subset:false first to avoid invisible glyphs
    if (hasCjk) {
      try {
        return await pdf.embedFont(loadedFont.bytes, { subset: false });
      } catch {
        console.warn(`[pdf-editing] Full font embed failed, trying subset. font=${loadedFont.path}`);
      }
    }

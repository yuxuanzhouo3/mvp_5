...
async function createDashScopeTextToVideoTask(modelId: string, prompt: string) {
  ...
  // Wan 2.2 T2V Plus supports video_length param (number of frames, must follow 4k+1 pattern)
  // 81 frames ≈ 5s, 129 frames ≈ 5-8s (max supported by Wan 2.2)
  const DASHSCOPE_T2V_VIDEO_LENGTH = 129;
  const inputCandidates: Record<string, unknown>[] = [
    {
      prompt: promptText,
    },
    ...
    try {
      return await createDashScopeAsyncTask(
        "/api/v1/services/aigc/video-generation/video-synthesis",
        {
          ...
          input,
          parameters: {
            video_length: DASHSCOPE_T2V_VIDEO_LENGTH,
          },
        },
      );
    } catch (error) {
